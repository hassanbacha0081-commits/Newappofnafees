      ...
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center bg-white p-4 rounded-xl border border-sky-200 shadow-sm gap-4">
        <div className="flex flex-wrap items-center gap-3 flex-1 max-w-xl">
          <div className="relative flex-1 min-w-[200px]">
            <input 
              type="text" 
              className="w-full p-3 pl-10 pr-10 bg-zinc-50 border border-sky-200 rounded-lg outline-none focus:border-gold text-black shadow-inner font-semibold text-sm"
              value={searchTerm}
              placeholder={lang === 'ur' ? 'نام، فون، مسئلہ یا آئٹم تلاش کریں...' : 'Search name, phone, issue, or item...'}
          ...
          {searchTerm && (
            <span className="bg-gold/10 text-gold-dark border border-gold/30 px-3 py-1.5 rounded-lg text-xs font-bold font-mono whitespace-nowrap">
              {lang === 'ur' ? `${repairs?.length || 0} ریکارڈز ملے` : `${repairs?.length || 0} records found`}
            </span>
          )}
        </div>
        <button 
